...
const static std::string kNullLightPath = "/dev/null";

static std::ofstream openOptionalLightNode(const std::string& path) {
    std::ofstream node(path);
    if (!node) {
        LOG(WARNING) << "Optional light node missing " << path << ", error=" << errno
                     << " (" << strerror(errno) << "), using " << kNullLightPath;
        return std::ofstream(kNullLightPath);
    }
...
int main() {
    ...
    std::ofstream nubiaBlink = openOptionalLightNode(kNubiaBlinkPath);
    std::ofstream nubiaBrightness = openOptionalLightNode(kNubiaBrightnessPath);
    std::ofstream nubiaFadeParam = openOptionalLightNode(kNubiaFadeParamPath);
    std::ofstream nubiaGradeParam = openOptionalLightNode(kNubiaGradeParamPath);
    std::ofstream nubiaOutn = openOptionalLightNode(kNubiaOutnPath);
